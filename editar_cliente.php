...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if (count($cambios) > 0) $accion = 'Contacto editado: Cambió ' . implode(', ', $cambios);
        
        $stmt = $pdo->prepare("UPDATE clientes SET documento=?, nombre=?, apellido=?, telefono=?, tipo=?, asignado=?, direccion=?, comentario=? WHERE id=?");
        $stmt->execute([$documento, $nombre, $apellido, $telefono, $tipo, $asignado, $direccion, $comentario, $id]);

        // Actualizar nombre en oportunidades
...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        .btn-update { height: 40px; padding: 0 20px; border: none; border-radius: 7px; background: #2563eb; color: white; font-size: 13px; font-weight: 600; transition: .2s; }
        .btn-update:hover { background: #1d4ed8; transform: translateY(-1px); }
...
<main class="main">
    <div class="page-container">
        <div class="breadcrumb-custom">Clientes / <span>Actualizar Cliente</span></div>
        <h1 class="page-title">Actualizar cliente</h1>
        <p class="page-description">Modifica la información registrada del cliente.</p>
            ...
            <div class="form-content">
                ...
                <form method="POST" id="formCliente">
                        ...
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    <input type="text" id="doc_numero" class="form-control" placeholder="Ej. 12345678" maxlength="11" value="<?php echo $cliente['documento'] ?? ''; ?>">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="hidden" name="documento" id="documento_hidden" value="<?php echo $cliente['documento'] ?? ''; ?>">
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="asignado" class="form-select">
                                    ...
                                    <?php foreach ($usuarios as $u): ?>
                                        <option value="<?php echo $u['nombre']; ?>" <?php if($cliente['asignado'] == $u['nombre']) echo 'selected'; ?>><?php echo $u['nombre']; ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                    ...
                    <div class="form-footer">
                        ...
                        <button type="submit" class="btn-update"><i class="bi bi-check-lg me-1"></i> Actualizar cliente</button>
                    </div>
                </form>
            </div>
        </div>
